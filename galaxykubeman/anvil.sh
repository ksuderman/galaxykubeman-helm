...
$(hi USAGE)
   ./anvil.sh [-s|--settings FILE] [-t|--tag TAG] [-p|--prefix PREFIX] [--as MIN MAX] (cluster|disks|namespace|helm|all|cleanup)
...
$(hi OPTIONS)
    ...
    $(hi --as)       enables and autoscaling cluster and sets the min and max nodes
...
function _helm() {
	echo "Running Helm"
	helm install -n $PREFIX $PREFIX-galaxy . \
    --wait\
    --timeout 900s\
