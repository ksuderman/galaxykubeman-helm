...
function cluster() {
	...
	if [[ $AUTO_SCALE = true ]] ; then
		gcloud container clusters create $PREFIX-cluster --cluster-version=$GKE_VERSION --disk-size=100 --num-nodes=1 --machine-type=$MACHINE --zone $ZONE --enable-autoscaling --min-nodes $MIN_NODES --max-nodes $MAX_NODES
	else
		gcloud container clusters create $PREFIX-cluster --cluster-version=$GKE_VERSION --disk-size=100 --num-nodes=1 --machine-type=$MACHINE --zone $ZONE
	fi
}

function disks() {
	echo "Creating disks"
	gcloud compute disks create "$PREFIX-postgres-pd" --size 10Gi --zone $ZONE
	gcloud compute disks create "$PREFIX-nfs-pd" --size 250Gi --zone $ZONE
}
...
function _helm() {
  local NAME="$PREFIX-galaxy"
    ...
    --set galaxy.postgresql.deploy=true \
...
function _helm2() {
	local NAME="$PREFIX-galaxy"
	helm install -n $PREFIX $NAME $KUBEMAN_REPO\
		  --wait\
		  --timeout 300s\
		  --set nfs.storageClass.name="nfs-$PREFIX"\
		  --set cvmfs.repositories.cvmfs-gxy-data-$PREFIX="data.galaxyproject.org"\
...
function cleanup() {
  set +e
	gcloud container clusters delete -q $PREFIX-cluster --zone $ZONE;
...
function print() {
	...
	echo KUBEMAN_REPO=$KUBEMAN_REPO
	echo
}
...
while [[ $# -gt 0 ]] ; do
	case $1 in
		cluster|disks|namespace|ns|helm|all|cleanup|print) ;;
		-s|--settings)
			shift
			if [[ ! -e $1 ]] ; then
...
while [[ $# -gt 0 ]] ; do
	case $1 in
		cluster|disks|all|cleanup|print)
		    $1
		    ;;
		ns|namespace)
		  namespace
		  ;;
		--settings|--tag|--prefix|-s|-t|-p)
			shift
			;;
		-a|--auto|--scale|--as)
			shift 2
